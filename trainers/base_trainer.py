    ...
    def _load_ckpt_quantile_dropout(self, name):
        ...
        if self.args.EXPER.resume == "load_voc":
            ...
            if self.experiment_name == "strongerv3_quantile_dropout2":
                ...
                ckptfile = torch.load(os.path.join('./checkpoints/gaussian_ensemble_1/', 'checkpoint-{}.pth'.format(name)))
            state_dict=ckptfile['state_dict']
            new_state_dict= {}
            for k, v in state_dict.items():
                #name=k #remove 'module.' of DataParallel
    ...
    def _train_epoch(self):
        ...
        for i, inputs in tqdm(enumerate(self.train_dataloader),total=len(self.train_dataloader)):
            ...
            if self.global_iter % 100 == 0:
                ...
                for k, v in self.logger_losses.items():
                    print(k, ":", v.get_avg())
            if self.args.EXPER.experiment_name == 'strongerv3_quantile_adjusted' or self.args.EXPER.experiment_name == 'strongerv3_quantile_dropout' or self.args.EXPER.experiment_name == 'strongerv3_quantile_dropout2' or self.args.EXPER.experiment_name == 'strongerv3_quantile_dropout3' or self.args.EXPER.experiment_name == 'strongerv3_quantile_dropout4':
