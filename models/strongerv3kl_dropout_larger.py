...
class StrongerV3KL_Dropout_Larger(nn.Module):
    def __init__(self,cfg):
        ...
        self.headslarge=nn.Sequential(OrderedDict([
            ('conv0',conv_bn_dropout(self.outC[0],625,kernel=1,stride=1,padding=0)),
            ('conv1', sepconv_bn_dropout(625, 1225, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv2', conv_bn_dropout(1225, 625, kernel=1,stride=1,padding=0)),
            ('conv3', sepconv_bn_dropout(625, 1225, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv4', conv_bn_dropout(1225, 625, kernel=1,stride=1,padding=0)),
        ]))
        self.detlarge=nn.Sequential(OrderedDict([
            ('conv5',sepconv_bn_dropout(625,1225,kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv6', conv_bias(1225, self.gt_per_grid*(self.numclass+5+4),kernel=1,stride=1,padding=0))
        ]))
        self.mergelarge=nn.Sequential(OrderedDict([
            ('conv7',conv_bn_dropout(625,312,kernel=1,stride=1,padding=0)),
            ('upsample0',nn.UpsamplingNearest2d(scale_factor=2)),
        ]))
        #-----------------------------------------------
        self.headsmid=nn.Sequential(OrderedDict([
            ('conv8',conv_bn_dropout(self.outC[1]+312,312,kernel=1,stride=1,padding=0)),
            ('conv9', sepconv_bn_dropout(312, 625, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv10', conv_bn_dropout(625, 312, kernel=1,stride=1,padding=0)),
            ('conv11', sepconv_bn_dropout(312, 625, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv12', conv_bn_dropout(625, 312, kernel=1,stride=1,padding=0)),
        ]))
        self.detmid=nn.Sequential(OrderedDict([
            ('conv13',sepconv_bn_dropout(312,625,kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv14', conv_bias(625, self.gt_per_grid*(self.numclass+5+4),kernel=1,stride=1,padding=0))
        ]))
        self.mergemid=nn.Sequential(OrderedDict([
            ('conv15',conv_bn_dropout(312,160,kernel=1,stride=1,padding=0)),
            ('upsample0',nn.UpsamplingNearest2d(scale_factor=2)),
        ]))
        #-----------------------------------------------
        self.headsmall=nn.Sequential(OrderedDict([
            ('conv16',conv_bn_dropout(self.outC[2]+160,160,kernel=1,stride=1,padding=0)),
            ('conv17', sepconv_bn_dropout(160, 312, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv18', conv_bn_dropout(312, 160, kernel=1,stride=1,padding=0)),
            ('conv19', sepconv_bn_dropout(160, 312, kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv20', conv_bn_dropout(312, 160, kernel=1,stride=1,padding=0)),
        ]))
        self.detsmall=nn.Sequential(OrderedDict([
            ('conv21',sepconv_bn_dropout(160,312,kernel=3, stride=1, padding=1,seprelu=cfg.seprelu)),
            ('conv22', conv_bias(312, self.gt_per_grid*(self.numclass+5+4),kernel=1,stride=1,padding=0))
        ]))
        if cfg.ASFF:
...
if __name__ == '__main__':
    ...
    assert 0
    model=net.eval()
    load_checkpoint(model,torch.load('checkpoints/coco625_prune/checkpoint-best.pth'))
