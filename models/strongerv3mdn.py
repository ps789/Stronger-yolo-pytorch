...
class StrongerV3MDN(nn.Module):
    def __init__(self,cfg):
        ...
        self.detlarge=nn.Sequential(OrderedDict([
            ...
            ('conv6', conv_bias(1024, self.gt_per_grid*(self.numclass+3*4*5+1),kernel=1,stride=1,padding=0))
        ]))
        ...
        self.detmid=nn.Sequential(OrderedDict([
            ...
            ('conv14', conv_bias(512, self.gt_per_grid*(self.numclass+3*4*5+1),kernel=1,stride=1,padding=0))
        ]))
        ...
        self.detsmall=nn.Sequential(OrderedDict([
            ...
            ('conv22', conv_bias(256, self.gt_per_grid*(self.numclass+1+3*4*5),kernel=1,stride=1,padding=0))
        ]))
        if cfg.ASFF:
    ...
    def decode(self,output,stride):
        ...
        output=output.view(bz,gridsize,gridsize,self.gt_per_grid,self.numclass+1+3*4*5)
        pi, x1y1,x2y2,variance,conf,prob=torch.split(output,[4*5, 2*5,2*5,4*5,1,self.numclass],dim=4)
        ...
        xy_grid=torch.stack([shiftx,shifty],dim=4).repeat(1, 1, 1, 1, 5).cuda()
    ...
    def decode_infer(self,output,stride):
        ...
        output=output.permute(0,2,3,1)
        output=output.view(bz,gridsize,gridsize,self.gt_per_grid,self.numclass+1+5*4*3)
        pi, x1y1,x2y2,variance,conf,prob=torch.split(output,[4*5, 2*5,2*5,4*5,1,self.numclass],dim=4)
