...
def loss_per_scale(conv, pred, label, bboxes, stride,cfg):
    ...
    # loss = torch.sum(loss)
    # return loss
def yololoss_quantile(
        cfg,conv_sbbox, conv_mbbox, conv_lbbox,#conv_sbbox_orig, conv_mbbox_orig, conv_lbbox_orig,
        pred_sbbox, pred_mbbox, pred_lbbox,#pred_sbbox_orig, pred_mbbox_orig, pred_lbbox_orig,
        label_sbbox, label_mbbox, label_lbbox,
...
def yololoss_quantile_adjusted(
    ...
    conf_loss=conf_loss_s+conf_loss_m+conf_loss_l
    probloss=probloss_s+probloss_m+probloss_l
    GIOUloss_s,conf_loss_s,probloss_s = loss_per_scale(conv_sbbox_orig, pred_sbbox_orig, label_sbbox, sbboxes,
                                strides[0],cfg=cfg)
    GIOUloss_m,conf_loss_m,probloss_m = loss_per_scale(conv_mbbox_orig, pred_mbbox_orig, label_mbbox, mbboxes,
                                strides[1],cfg=cfg)
    GIOUloss_l,conf_loss_l,probloss_l = loss_per_scale(conv_lbbox_orig, pred_lbbox_orig, label_lbbox, lbboxes,
                                strides[2],cfg=cfg)
